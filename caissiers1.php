...
$sqlAcc=$connexion->query("SELECT *FROM accountants c WHERE c.id_u='$id'");
...
if($connect==true AND ($comptidM!=0 OR $comptidA!=0 OR $comptidAcc!=0))
{
if($status==1)
{
        ...
        <div class="container">
            ...
            <div class="nav-collapse">
                ...
                <ul class="nav pull-right">
                    <li class="">
                        <form method="post" action="caissiers1.php?<?php if(isset($_GET['codeCash'])){ echo 'codeCash='.$_GET['codeCash'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
                            ...
                            {
                                ?>
                                <a href="caissiers1.php?english=english<?php if(isset($_GET['codeCash'])){ echo '&codeCash='.$_GET['codeCash'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divCash'])){ echo '&divCash='.$_GET['divCash'];}?>" class="btn"><?php echo getString(30);?></a>
                                <?php
                            }else{
                                ?>
                                <a href="caissiers1.php?francais=francais<?php if(isset($_GET['codeCash'])){ echo '&codeCash='.$_GET['codeCash'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divCash'])){ echo '&divCash='.$_GET['divCash'];}?>" class="btn"><?php echo getString(29);?></a>
                                <?php
                            }
                            ?>
                            <br/>
...
<div class="account-container" style="width:90%; text-align:center;">
            ...
            $('#q').keyup( function(){
                ...
                if( $field.val().length > 0 )
                {
                    ...
                    $.ajax({
                        ...
                        url : 'traitement_caissiers1.php?name=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
            ...
            $('#s').keyup( function(){
                ...
                if( $field.val().length > 0 )
                {
                    ...
                    $.ajax({
                        ...
                        url : 'traitement_caissiers1.php?sn=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
    ...
    if(isset($_GET['divCash']))
    {
        ...
        if($comptidA!=0)
        {
            $resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND c.id_u=:idCash AND u.id_u=:idCash AND c.codecashier LIKE \'raC%\' ');
        ...
        if($comptidM!=0)
        {
            $resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND c.id_u=:idCash AND u.id_u=:idCash');
        ...
        if($comptidM!=0)
        {
            ...
            <table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
                ...
                <tr>
                    ...
                    <th style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">STATUS</th>
                </thead>
                <tbody>
                <?php
                while($ligneM=$resultatsM->fetch())
                {
                    ...
                    <tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>">
                        <td><?php echo $ligneM->codecashier;?></td>
                        <td><?php echo $ligneM->nom_u.' '.$ligneM->prenom_u;?></td>
                        <td>
                            <?php
                            $getcodeRec=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio=:codeC');

                            $getcodeRec->execute(array(
                                'codeC'=>$ligneM->codecashier
                            ))or die( print_r($connexion->errorInfo()));

                            $getcodeRec->setFetchMode(PDO::FETCH_OBJ);

                            $comptidRec=$getcodeRec->rowCount();
                        ...
                        <td><?php echo $ligneM->dateaffectationcash;?></td>

                        <?php
                        if($ligneM->status==1)
                        {
                            ?>
                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="utilisateurs.php?iduti=<?php echo $ligneM->id_u?>&updatecash=<?php echo $ligneM->codeR;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
                            </td>

                            <td>
                                <a href="report.php?cash=<?php echo $ligneM->codecashier;?><?php if(isset($_GET['reporthospCash'])){ echo '&reporthospCash='.$_GET['reporthospCash'];}else{ if(isset($_GET['report'])){ echo '&report='.$_GET['report'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneM->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
                            </td>

                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codecashier;?>&updatecash=ok&divCash=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
                            ...
                            <td></td>
                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codecashier;?>&updatecash=ok&divCash=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
        ...
        if($comptidA!=0)
        {
            ...
            <table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
                ...
                <tbody>
                <?php
                while($ligneA=$resultatsA->fetch())
                {
                    ?>
                    <tr style="text-align:center;<?php if($ligneA->status==0){?>background:rgb(253,168,170)<?php ;}?>">
                        <td><?php echo $ligneA->codecashier;?></td>
                        <td><?php echo $ligneA->nom_u.' '.$ligneA->prenom_u;?></td>
                        <td>
                            <?php
                            $getcodeRec=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio=:codeC');

                            $getcodeRec->execute(array(
                                'codeC'=>$ligneA->codecashier
                            ))or die( print_r($connexion->errorInfo()));
                        ...
                        <td><?php echo $ligneA->dateaffectationcash;?></td>

                        <td>
                            <a href="report.php?cash=<?php echo $ligneA->codecashier;?>&audit=<?php echo $_SESSION['id'];?><?php if(isset($_GET['reporthospCash'])){ echo '&reporthospCash='.$_GET['reporthospCash'];}else{ if(isset($_GET['report'])){ echo '&report='.$_GET['report'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneA->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
                        </td>
        ...
        <a href="caissiers1.php?iduser=<?php echo $id;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show Caissiers" class="btn-large">Show Caissiers</a>
        <?php
    }
    ?>
    ...
    if(!isset($_GET['divCash']))
    {
        ...
        <table class="tablesorter" cellspacing="0">
            ...
            try
            {
                ...
                $resultats=$connexion->query('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND (c.codecashier LIKE \'CSTPC%\' OR c.codecashier LIKE \'CSTPR%\') ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
                ...
                while($ligne=$resultats->fetch())//on recupere la liste des éléments
                {
                    ...
                    <tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>">
                        <td><?php echo $ligne->codecashier;?></td>
                        <td><?php echo $ligne->nom_u.' '.$ligne->prenom_u;?></td>
                        <td>
                            <?php
                            $getcodeRec=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio=:codeC');

                            $getcodeRec->execute(array(
                                'codeC'=>$ligne->codecashier
                            ))or die( print_r($connexion->errorInfo()));
                        ...
                        <td><?php echo $ligne->dateaffectationcash;?></td>
                        ...
                        if($ligne->status==1 AND $comptidA==0)
                        {
                            ?>
                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="utilisateurs.php?iduti=<?php echo $ligne->id_u?>&updatecash=<?php echo $ligne->codeR;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
                            </td>

                            <td>
                                <a href="report.php?cash=<?php echo $ligne->codecashier;?><?php if(isset($_GET['reporthospCash'])){ echo '&reporthospCash='.$_GET['reporthospCash'];}else{ if(isset($_GET['report'])){ echo '&report='.$_GET['report'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
                            </td> 

                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="report.php?cashier=<?php echo $ligne->codecashier;?>&id_u=<?php echo $ligne->id_u;?>&UnBilled=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn " ><?php echo 'UnBilled Bill';?></a>
                            </td>

                            <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                <a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codecashier;?>&updatecash=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
                            ...
                            if($ligne->status==0 AND $comptidA==0)
                            {
                                ?>
                                <td>
                                    <a href="report.php?cash=<?php echo $ligne->codecashier;?><?php if(isset($_GET['reporthospCash'])){ echo '&reporthospCash='.$_GET['reporthospCash'];}else{ if(isset($_GET['report'])){ echo '&report='.$_GET['report'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn"><?php echo getString(94);?></a>
                                </td> 

                                <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                    <a href="report.php?cashier=<?php echo $ligne->codecashier;?>&id_u=<?php echo $ligne->id_u;?>&UnBilled=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn " ><?php echo 'UnBilled Bill';?></a>
                                </td>

                                <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                    <a href="report.php?cash=<?php echo $ligne->codecashier;?>&audit=<?php echo $_SESSION['id'];?><?php if(isset($_GET['reporthospCash'])){ echo '&reporthospCash='.$_GET['reporthospCash'];}else{ if(isset($_GET['report'])){ echo '&report='.$_GET['report'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
                                </td>

                                <td style="<?php if(isset($_SESSION['codeAcc'])){echo "display: none;";} ?>">
                                    <a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codecashier;?>&updatecash=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
        ...
        <tr>
            <td>
                ...
                $nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND (c.codecashier LIKE \'CSTPC%\' OR c.codecashier LIKE \'CSTPR%\') ORDER BY u.nom_u ');
