...
if($connected==true AND isset($_SESSION['id']))
{
	if(isset($_SESSION['codeL']))
	{
		...
		if(isset($_SESSION['codeC']))
		{
			...
			{
				$resultatsLabo=$connexion->prepare('SELECT *FROM utilisateurs u, stockkeeper k WHERE u.id_u=k.id_u and k.id_u=:operation');
				$resultatsLabo->execute(array(
				'operation'=>$idLabo	
				));
				$resultatsLabo->setFetchMode(PDO::FETCH_OBJ);
			...
			?>
				...
				<thead>
					...
					<tr>
						...
						<th style="width:8%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Valeur (min-max)';?></th>
				...
				while($ligneSouscategoPrestaLabo=$resultatsSouscategoPrestaLabo->fetch())
				{
					...
					if($ligneNomSouscatego=$getNomSouscatego->fetch())
					{
						...
						$nomSousCatego='Other category';
					}
				?>
					
					<tr style="background:#bbb">
						<td colspan=11><?php echo "Other category";?>
						</td>
					</tr>
...
<!-- 							<td style="text-align:center; ">
									...
									?>
											...
											<table class="printPreview tablesorter3" cellspacing="0" style="width:100%;">
												<tr>
													...
													?>
													( 
													<span type="text" id="min<?php echo $v;?>" name="min[]" style="height:15px; width:130px; font-weight:bold; font-size:10px;"><?php if($minresultats = $ligneValeur->min_valeur !=""){ echo $minresultats = $ligneValeur->min_valeur;}?></span> 
													- 
													<span type="text" id="max<?php echo $v;?>" name="max[]" style="height:15px; width:130px; font-weight:bold; font-size:10px;"><?php if($maxresultats = $ligneValeur->max_valeur !=""){ echo $maxresultats = $ligneValeur->max_valeur;}?></span> )
													<?php
													}
													?>
													</td>						
	...
	if(isset($_GET['laboreport']))
	{
		...
		if(isset($_GET['divPersoMedicReport']))
		{
			...
			if (!isset($_SESSION['codeC']) && !isset($_GET['alllaborepo'])) {
	?>
			<div id="divPersoMedicReport">
				...
				?>
					...
					if(isset($_SESSION['codeS'])){
						$resultatsSouscategoPrestaLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE  ml.examenfait=1 '.$dailydateperso.' ORDER BY ml.dateresultats');
						$resultatsSouscategoPrestaLabo->execute(array(
							'id_uL'=>$_SESSION['id']
						));
					}else{
						$resultatsSouscategoPrestaLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.id_uL=:id_uL AND ml.examenfait=1 '.$dailydateperso.' ORDER BY ml.dateresultats');
						$resultatsSouscategoPrestaLabo->execute(array(
							'id_uL'=>$_SESSION['id']
						));
					}
					...
					while($ligneSouscategoPrestaLabo=$resultatsSouscategoPrestaLabo->fetch())
					{
						...
						$selectInfoPresta = $connexion->prepare('SELECT * FROM '.$presta_assuLab.' WHERE id_prestation=:id_prestationExa');
						$selectInfoPresta->execute(array(
							'id_prestationExa'=>$ligneSouscategoPrestaLabo->id_prestationExa
						));
						$selectInfoPresta->setFetchMode(PDO::FETCH_OBJ);
						$ligneInfoPresta = $selectInfoPresta->fetch();

						$idcatego=$ligneInfoPresta->id_categopresta;
						$idsouscatego=$ligneInfoPresta->id_souscategopresta;
					...
					?>
						...
						{*/
							...
							if($ligneSouscategoPrestaLabo->moreresultats!=0)
							{
						?>
							...
							<tr style="text-align:center;">
								...
								<?php
									...
									'operation'=>$ligneSouscategoPrestaLabo->numero
									));
									...
									{
										$fullnamePa = $lignePatient->full_name.' ('.$ligneSouscategoPrestaLabo->numero.')';
										
										if($fullnamePaCheck!=$fullnamePa)
										{
											echo $lignePatient->full_name.'<br/>('.$ligneSouscategoPrestaLabo->numero.')';
										}
									}else{
										$fullnamePa="";
										echo '';
								...
								<td style="text-align:center;font-weight:600;">
								<?php
								if($ligneSouscategoPrestaLabo->dateresultats != '0000-00-00')
								{
									$dateresultats=date('d-M-Y',strtotime($ligneSouscategoPrestaLabo->dateresultats));
									echo $dateresultats;
								}
								?>
								</td>
								...
								<td style="text-align:center;font-weight:bold;">
									...
									$resultPresta=$connexion->prepare('SELECT *FROM '.$presta_assuLab.' p WHERE p.id_prestation=:prestaId');		
									$resultPresta->execute(array(
									'prestaId'=>$ligneSouscategoPrestaLabo->id_prestationExa
									));
									...
									if($lignePresta=$resultPresta->fetch())//on recupere la liste des éléments
									{
										if($lignePresta->namepresta!='')
										{
											$examen=$lignePresta->namepresta;
										}else{								
											$examen=$lignePresta->nompresta;
										}
										...
										$examen=$ligneSouscategoPrestaLabo->autreExamen;
										$mesure='';
									}
									echo $examen;
									?>
								...
								<td style="text-align:center;">
								<?php 
								if($ligneSouscategoPrestaLabo->resultats!="")
								{
									$fichierjoint='Un fichier a été joint';
									echo 'Un fichier a été joint sur ces résultats';
								}else{
								...
								<td style="text-align:center;">
								<?php 
									$dateconsu=date('d-M-Y',strtotime($ligneSouscategoPrestaLabo->dateconsu));
									echo date('d-M-Y',strtotime($ligneSouscategoPrestaLabo->dateconsu));
								?>
								</td>
								...
								$resultatsMed=$connexion->prepare('SELECT *FROM utilisateurs u, medecins m WHERE u.id_u=m.id_u AND m.id_u=:idMed') or die( print_r($connexion->errorInfo()));
								$resultatsMed->execute(array(
								'idMed'=>$ligneSouscategoPrestaLabo->id_uM
								));
								...
								<td style="text-align:center;font-weight:normal;" colspan=2>
								<?php
								$idLabo=$ligneSouscategoPrestaLabo->id_uL;
								...
								<?php
								if($ligneSouscategoPrestaLabo->moreresultats==1)
								{
									$moreExaResuLab=array();
								...
								<tr>							
									...
									<table class="printPreview tablesorter3" style="width:100%">
										...
										$resultMoreMedLabo->execute(array(
										'idmedLab'=>$ligneSouscategoPrestaLabo->id_medlabo
										));
								...
								if($ligneSouscategoPrestaLabo->moreresultats==2)
								{
									$moreexamen='';
									$moreExaResuLab=array();
									$moreMinResuLab=array();
							...
							<tr>
								...
								{
									$dateresultats=date('d-M-Y',strtotime($ligneSouscategoPrestaLabo->dateresultats));
									echo date('d-M-Y',strtotime($ligneSouscategoPrestaLabo->dateresultats));
								}
								?>
								</td>
								...
								<td style="text-align:center;font-weight:bold;">
									...
									if($ligneSouscategoPrestaLabo->autreExamen=="")
									{
										if($ligneInfoPresta->namepresta!='')
										{
											$examen=$ligneInfoPresta->namepresta;
											$moreexamen="";					
										}else{
										
											$examen=$ligneInfoPresta->nompresta;
											$moreexamen="";
										}
										
										if($ligneInfoPresta->mesure!='')
										{
											$mesure=' [<span style="font-size:80%; font-weight:normal;">'.$ligneInfoPresta->mesure.'</span>]';
										}else{
											$mesure='';
										}
									}else{
								...
								<td style="text-align:center;font-weight:bold;font-style:italic;">
								<?php
									$resultats=$ligneSouscategoPrestaLabo->autreresultats;
									echo $ligneSouscategoPrestaLabo->autreresultats.''.$mesure;
								?>
								</td>
				...
				<table cellspacing="0" style="margin:10px auto auto; background:#fff; width:100%;">
					...
					<thead>
						...
						<tr>
							...
							<?php if(isset($_SESSION['codeC'])){?>
								<th style="width:5%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Assurance';?></th>
							<?php }?>
						...
						*/
						...
						$comptLaboPa=$resultLaboPa->rowCount();
							...
							while($ligneLaboPa=$resultLaboPa->fetch())
							{
								...
								for($i=1;$i<=$assuCount;$i++)
								{
									...
									if($ligneNomAssu=$getAssuConsu->fetch())
									{
										$assu = $ligneNomAssu->nomassurance;
										$presta_assuLab='prestations_'.$ligneNomAssu->nomassurance;
									}
								}
						...
						?>						
								<tr>
									...
									<td style="text-align:center;<?php if(!isset($_SESSION['codeC'])){echo"display: none;";}?>"><?php echo $assu; ?></td>
									...
									<td>
									<?php
									$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.id_consuLabo=:idMedLabo AND ml.prixprestationExa!=0.01 ORDER BY ml.id_medlabo');		
	...
	<div id="divPersoMedicReport">
				...
				?>
					...
					<thead>
						...
						<tr>
							...
							<th style="width:8%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Date Résultats';?></th>
							<th style="width:8%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Laboratory tests';?></th>
							<th style="width:5%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Doctor Name';?></th>
						</tr> 
				...
				<tbody>
					<?php
						...
						$TotalGnlPrice=0;
						//echo $dailydateperso;
						$resultLaboPa=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.examenfait=1 AND ml.id_uL=:id_uL '.$dailydateperso.' GROUP BY ml.id_consuLabo ORDER BY ml.dateresultats');		
						 $resultLaboPa->execute(array(
						'id_uL'=>$_SESSION['id']
						));
						...
						?>						
								<tr>
									...
									$resultMedLabo=$connexion->query( "SELECT *FROM utilisateurs u, medecins l WHERE u.id_u=l.id_u AND l.id_u=$idMed");
									$resultMedLabo->setFetchMode(PDO::FETCH_OBJ);
									if( $ligneMedId = $resultMedLabo->fetch())
									{
										$fullnameMed=$_SESSION['nom'].' '.$_SESSION['prenom'];
										echo $ligneMedId->full_name;
			...
			<div id="divPersoMedicReport">
				...
				?>
					...
					while($ligneSouscategoPrestaLabo=$resultatsSouscategoPrestaLabo->fetch())
					{
						...
						$getNomSouscatego->setFetchMode(PDO::FETCH_OBJ);
					    $coun=$getNomSouscatego->rowCount();
						...
						?>
							...
							<td colspan=11 style="font-weight: bold;font-size: 10px;"><?php echo 'Autres Category';?>
							</td>
						</tr>
							<?php
	...
	<?php if(isset($_GET['laboreportHosppeeked'])){?>
				...
				$resultLaboPa=$connexion->prepare('SELECT *FROM med_labo_hosp ml WHERE ml.examenfait=1 AND ml.id_uL=:id_uL '.$dailydateperso.' GROUP BY ml.id_hospLabo ORDER BY ml.dateresultats');		
				 $resultLaboPa->execute(array(
				'id_uL'=>$_SESSION['id']
				));
				
				$resultLaboPa->setFetchMode(PDO::FETCH_OBJ);
				//echo 'SELECT *FROM med_labo_hosp ml WHERE ml.id_uL=:id_uL AND ml.examenfait=1 '.$dailydateperso.' GROUP BY ml.id_hospLabo ORDER BY ml.dateresultats';
					...
					<table class="printPreview tablesorter3" cellspacing="0" style="width:100%;padding:0px;font-size:90%;">
						...
						?>						
								<tr>
										...
										?>
											<tr style="text-align:center;">
												...
												if($lignePresta=$resultPresta->fetch())
												{
													...
													// { 
													?>
														<span style="height:15px; width:130px; font-weight:bold; font-size:10px;"><?php echo $ligneMedLabo->autreresultats;?></span>
													<?php 
													// }
													...
													?>
													( 
													<span style="height:15px; width:130px; font-weight:bold; font-size:10px;"><?php if($ligneValeur->min_valeur !=""){ echo $ligneValeur->min_valeur;}else{echo "---";}?></span> 
													- 
													<span style="height:15px; width:130px; font-weight:bold; font-size:10px;"><?php if($ligneValeur->max_valeur !=""){ echo $ligneValeur->max_valeur;}else{echo "----";}?></span> )
													<?php
													}else{echo "----";}
													?>
									...
									<td style="text-align:center;"><?php echo date('d-M-Y',strtotime($ligneLaboPa->datehosp));?></td>
