...
if($comptidBill != 0)
{
	...
	if($ligne->codecashierHosp!="")
	{
		...
		$researchjourfactDoneby=$connexion->prepare('SELECT *FROM jourhosp_fact jhf WHERE jhf.id_hosp=:idhosp AND jhf.numbill=:numbill ORDER BY jhf.numbill LIMIT 1');
		$researchjourfactDoneby->execute(array(
			'idhosp'=>$_GET['idhosp'],
			'numbill'=>$numbill
		));
		
		$researchjourfactDoneby->setFetchMode(PDO::FETCH_OBJ);

		$comptidJHFBill=$researchjourfactDoneby->rowCount();

		if ($comptidJHFBill!=0) {
			
			while ($ligneJHF=$researchjourfactDoneby->fetch()) {
				$idDoneby=$ligneJHF->codecashier;
...
if($connected==true AND isset($_SESSION['codeCash']))
{
	...
	<table style="width:100%">
		...
		$resultHosp=$connexion->prepare('SELECT *FROM patients_hosp ph, jourhosp_fact jhf WHERE ph.id_hosp=:hospId AND ph.id_hosp=jhf.id_hosp AND ph.numero=jhf.numroom AND ph.numero=:num AND ph.dateEntree=:datehosp AND ph.id_assuHosp=:idassu AND jhf.numbill=:numbill ORDER BY ph.id_hosp');
		...
		/*-----------------------Restauration------------------*/
		if (isset($_GET['idresto'])) {
			$resultResto=$connexion->prepare('SELECT *FROM restauration r, jourhosp_fact jhf WHERE r.id_resto=:restoId AND r.id_resto=jhf.id_resto AND r.numero=jhf.numero AND r.numero=:num AND r.dateEntree=:datehosp AND r.id_assuHosp=:idassu AND jhf.numbill=:numbill ORDER BY r.id_hosp');
			...
			$comptResto=$resultHosp->rowCount();
		...
		/*------------------------Tour de salle----------------*/
		if (isset($_GET['idtourdesalle'])) {
			$resultTds=$connexion->prepare('SELECT *FROM tour_de_salle tds, jourhosp_fact jhf WHERE tds.id_tour_de_salle=:tdsId AND tds.id_tour_de_salle=jhf.id_tour_de_salle AND tds.numero=jhf.numero  AND tds.numero=:num AND tds.dateEntree=:datehosp AND tds.id_assuHosp=:idassu AND jhf.numbill=:numbill ORDER BY tds.id_hosp');
			$resultTds->execute(array(
				'tdsId'=>$idtourdesalle,
				'num'=>$numPa,
				'idassu'=>$idassurance,
				'numbill'=>$numbill,
			...
			$resultTds->setFetchMode(PDO::FETCH_OBJ);
			
			$comptTds=$resultHosp->rowCount();
		...
		$resultMedConsult=$connexion->prepare('SELECT *FROM med_consult_hosp mc, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mc.numero AND mc.numero=jhf.numero AND mc.numero=:num AND mc.id_hospMed=:idconsu AND p.id_hosp=mc.id_hospMed AND mc.id_hospMed=jhf.id_hosp AND mc.datehosp!="0000-00-00" AND mc.id_factureMedConsu!="" AND mc.id_factureMedConsu=:numbill AND mc.id_prestationConsu!=1068 AND mc.id_prestationConsu!=984 AND mc.id_prestationConsu!=1216 AND mc.id_prestationConsu!=983 AND mc.id_prestationConsu!=1143 AND mc.id_prestationConsu!=1144 ORDER BY mc.id_medconsu');
		...
		$resultMedInf=$connexion->prepare('SELECT *FROM med_inf_hosp mi, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mi.numero AND p.numero=jhf.numero AND mi.numero=:num AND mi.id_hospInf=:idhosp AND p.id_hosp=mi.id_hospInf AND p.id_hosp=jhf.id_hosp AND mi.id_factureMedInf!="" AND mi.id_factureMedInf=:numbill ORDER BY mi.id_medinf');
		...
		$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo_hosp ml, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=ml.numero AND p.numero=jhf.numero AND ml.numero=:num AND ml.id_hospLabo=:idhosp AND p.id_hosp=ml.id_hospLabo AND p.id_hosp=jhf.id_hosp AND ml.id_factureMedLabo!="" AND ml.id_factureMedLabo=:numbill AND ml.id_factureMedLabo=jhf.numbill ORDER BY ml.id_medlabo');
		...
		$resultMedRadio=$connexion->prepare('SELECT *FROM med_radio_hosp mr, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mr.numero AND p.numero=jhf.numero AND mr.numero=:num AND jhf.numero=:num AND mr.id_hospRadio=:idhosp AND p.id_hosp=mr.id_hospRadio AND p.id_hosp=jhf.id_hosp AND mr.id_factureMedRadio!="" AND mr.id_factureMedRadio=:numbill AND jhf.numbill=:numbill ORDER BY mr.id_medradio');
		...
		$resultMedKine=$connexion->prepare('SELECT *FROM med_kine_hosp mk, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mk.numero AND p.numero=jhf.numero AND mk.numero=:num AND mk.id_hospKine=:idhosp AND p.id_hosp=mk.id_hospKine AND p.id_hosp=jhf.id_hosp AND mk.id_factureMedKine!="" mk.id_factureMedKine=:numbill AND jhf.numbill=mk.id_factureMedKine ORDER BY mk.id_medKine');
		...
		$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom_hosp mco, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mco.numero AND p.numero=jhf.numero AND mco.numero=:num AND mco.id_hospConsom=:idhosp AND p.id_hosp=mco.id_hospConsom AND p.id_hosp=jhf.id_hosp AND mco.id_factureMedConsom!="" AND mco.id_factureMedConsom=:numbill AND jhf.numbill=mco.id_factureMedConsom ORDER BY mco.id_medconsom');
		...
		$resultMedMedoc=$connexion->prepare('SELECT *FROM med_medoc_hosp mdo, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mdo.numero AND p.numero=jhf.numero AND mdo.numero=:num AND mdo.id_hospMedoc=:idhosp AND p.id_hosp=mdo.id_hospMedoc AND p.id_hosp=jhf.id_hosp AND mdo.id_factureMedMedoc!="" AND mdo.id_factureMedMedoc=:numbill AND jhf.numbill=mdo.id_factureMedMedoc ORDER BY mdo.id_medmedoc');
		...
		$resultMedOrtho=$connexion->prepare('SELECT *FROM med_ortho_hosp mo, patients_hosp p, jourhosp_fact jhf WHERE p.numero=:num AND p.numero=mo.numero AND p.numero=jhf.numero AND mo.numero=:num AND mo.id_hospOrtho=:idhosp AND p.id_hosp=mo.id_hospOrtho AND p.id_hosp=jhf.id_hosp AND mo.id_factureMedOrtho!="" AND mo.id_factureMedOrtho=:numbill AND jhf.numbill=mo.id_factureMedOrtho ORDER BY mo.id_medOrtho');
		...
		try
		{
							...
							if($comptPresta!=0)
							{
								if($lignePresta=$resultPresta->fetch())
								{
									...
									<td style="text-align:center;">
										...
										/*$selectjourHosp_fact=$connexion->prepare('SELECT * FROM jourhosp_fact WHERE id_hosp=:idhosp AND numero=:numPa');
										$selectjourHosp_fact->execute(array(
											'idhosp'=>$ligneHosp->id_hosp,
											'numPa'=>$ligneHosp->numero
										));
										$selectjourHosp_fact->setFetchMode(PDO::FETCH_OBJ);

										$comptselectjourHosp_fact=$selectjourHosp_fact->rowCount();
										//echo "comptselectjourHosp_fact".$comptselectjourHosp_fact;
										
										if($comptselectjourHosp_fact!=0)
										{
											$datediff = 0;
											for ($i=1; $i<=$comptselectjourHosp_fact ; $i++) { 
												$jourHosp_fact = $selectjourHosp_fact->fetch();
												$datedebut = $jourHosp_fact->datedebut;
												$datefin = $jourHosp_fact->datefin;

												$dateIn=strtotime($ligneHosp->dateEntree);
												$dateOut=strtotime($ligneHosp->dateSortie);

												if ($datefin == $datedebut) {
													$datediffx= abs($dateOut - $dateIn)- 1;
												}else{
													$datediffx= abs($dateOut - $dateIn)- abs($datefin-$datedebut);
												}
												$datediff+=$datediffx;
											}
										}else{*/
									...
									<!-- <td class="buttonBill">
										<?php
										if($ligneHosp->statusPaHosp!=1 || $comptselectjourHosp_fact!=0)
										{
											if ($comptselectjourHosp_fact!=0) {
												/*$jourHosp_factpa = $selectjourHosp_fact->fetch();
												$datedebut = $jourHosp_factpa->datedebut;
												$datefin = $jourHosp_factpa->datefin;*/

												if ($datefin == $datedebut) {
													echo '<span style="font-size:100%; font-weight:bold;">Payed 1 day</span>';
												}else{
													$datedifffact =abs($datefin-$datedebut);
													echo '<span style="font-size:100%; font-weight:bold;">Payed '.$datedifffact.' day</span>';
												}
											}else{
			...
			<table class="printPreview tablesorter3" cellspacing="0" style="margin:0 auto 25px;width:90%;">
				...
				<tbody>
					...
					$comptPresta=$resultResto->rowCount();
					...
					<tr style="font-weight:bold;">
						...
						<td style="text-align:center;">
						<?php
							echo date('d-M-Y', strtotime($ligneResto->dateEntree));
						?>
						</td>
						<td style="text-align:center;">
						<?php
							echo date('d-M-Y', strtotime($ligneResto->dateSortie));
						?>
						</td>
						...
						<?php
						//$connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
						/*$selectjourResto_fact=$connexion->prepare('SELECT * FROM jourhosp_fact WHERE id_resto=:idresto AND numero=:numPa');
						$selectjourResto_fact->execute(array(
							'idresto'=>$ligneResto->id_resto,
							'numPa'=>$ligneResto->numero
						));
						$selectjourResto_fact->setFetchMode(PDO::FETCH_OBJ);

						$comptselectjourResto_fact=$selectjourResto_fact->rowCount();
						//echo "comptselectjourHosp_fact".$comptselectjourHosp_fact;
						
						if($comptselectjourResto_fact!=0)
						{
							$datediffresto = 0;
							$diffresto = 0;
							for ($i=1; $i<=$comptselectjourResto_fact; $i++) {
								$jourResto_fact = $selectjourResto_fact->fetch();
								$datedebut = $jourResto_fact->datedebut;
								$datefin = $jourResto_fact->datefin;

								$dateIn=strtotime($ligneResto->dateEntree);
								$dateOut=strtotime($ligneResto->dateSortie);
								...
								if ($datefin == $datedebut) {
									...
									$datediffx= abs($dateOut - $dateIn)- abs($datefin-$datedebut);
									$difffjourResto = abs($datefin-$datedebut);
								}
								$datediffresto += $datediffx;
								$diffresto+= $difffjourResto;
							}							
						}else{*/
							$dateIn=strtotime($ligneResto->dateEntree);
								$dateOut=strtotime($ligneResto->dateSortie);
							
							$datediffresto= abs($dateOut - $dateIn);
						//}
						
						$nbrejrs= floor($datediffresto /(60*60*24));
						...
						<td style="text-align:center;">
                            <?php
                            $topupResto = $balanceHospCCO - $balanceHosp;
						...
						<!-- <td class="buttonBill">
						<?php
						if($ligneResto->statusPaResto!=1 || $comptselectjourResto_fact!=0)
						{
							if ($comptselectjourResto_fact!=0) {
								echo '<span style="font-size:100%; font-weight:bold;">Payed '.$diffresto
									.' day</span>';
							}else{
								echo '<span style="font-size:100%; font-weight:bold;">Payed</span>';
							}
				...
				<?php
					...
					<tr style="font-weight:bold;">
						<td style="text-align:center;">
							...
							if($ligneResto->dateSortie>='0000-00-00')
							{
								echo date('d-M-Y', strtotime($ligneResto->dateSortie));
							}
							?>
				...
				<tbody>
					...
					<tr style="font-weight:bold;">
						<td style="text-align:center;">
							...
							<td style="text-align:center;">
							<?php
								echo date('d-M-Y', strtotime($ligneTourdesalle->dateEntree));
							?>
							</td>
							<td style="text-align:center;">
							<?php
							...
							if($ligneTourdesalle->dateSortie>='0000-00-00')
							{
								echo date('d-M-Y', strtotime($ligneTourdesalle->dateSortie));
							}
							?>
							</td>
							...
							<?php
							//$connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
							/*$selectjourTDS_fact=$connexion->prepare('SELECT * FROM jourhosp_fact WHERE id_tour_de_salle=:idtourdesalle AND numero=:numPa');
							$selectjourTDS_fact->execute(array(
								'idtourdesalle'=>$ligneTourdesalle->id_tour_de_salle,
								'numPa'=>$ligneTourdesalle->numero
							));
							$selectjourTDS_fact->setFetchMode(PDO::FETCH_OBJ);

							$comptselectjourTDS_fact=$selectjourTDS_fact->rowCount();
							//echo "comptselectjourHosp_fact".$comptselectjourHosp_fact;
							
							if($comptselectjourTDS_fact!=0)
							{
								$datedifftds = 0;
								$difftds=0;
								for ($i=0; $i <$comptselectjourTDS_fact ; $i++) { 
									$jourTDS_fact = $selectjourTDS_fact->fetch();
									$datedebut = $jourTDS_fact->datedebut;
									$datefin = $jourTDS_fact->datefin;

									$dateIn=strtotime($ligneTourdesalle->dateEntree);
									$dateOut=strtotime($ligneTourdesalle->dateSortie);

									if ($datefin == $datedebut) {
										$datediff= abs($dateOut - $dateIn)- 1;
										$diffjourtds = 1;
									}else{
										$datediff= abs($dateOut - $dateIn)- abs($datefin-$datedebut);
										$diffjourtds = abs($datefin-$datedebut);
									}
									$datedifftds+=$datediff;
									$difftds+=$diffjourtds;
								}
							}else{*/
								$dateIn=strtotime($ligneTourdesalle->dateEntree);
							...
							<!-- <td class="buttonBill">
							<?php
							if($ligneTourdesalle->statusPa!=1 || $comptselectjourTDS_fact!=0)
							{
								if ($ligneTourdesalle->statusPa!=1) {
										echo '<span style="font-size:100%; font-weight:bold;">Payed '.$difftds.' day</span>';
								}else{
									echo '<span style="font-size:100%; font-weight:bold;">Payed</span>';
								}
							}
