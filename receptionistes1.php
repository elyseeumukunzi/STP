...
<head>
	<title>AFFICHE RECEPTIONISTES</title>
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="receptionistes1.php?<?php if(isset($_GET['codeRec'])){ echo 'codeRec='.$_GET['codeRec'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
					...
					{
					?>
						<a href="receptionistes1.php?english=english<?php if(isset($_GET['codeRec'])){ echo '&codeRec='.$_GET['codeRec'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divRec'])){ echo '&divRec='.$_GET['divRec'];}?>" class="btn"><?php echo getString(30);?></a>
					<?php
					}else{
					?>
						<a href="receptionistes1.php?francais=francais<?php if(isset($_GET['codeRec'])){ echo '&codeRec='.$_GET['codeRec'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divRec'])){ echo '&divRec='.$_GET['divRec'];}?>" class="btn"><?php echo getString(29);?></a>
					<?php
					}					
					?>
						<br/>						
...
$(document).ready( function() {
  ...
  $('#q').keyup( function(){
	...
	url : 'traitement_receptionistes1.php?name=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
	...
	url : 'traitement_receptionistes1.php?sn=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
	...
	if(isset($_GET['divRec']))
	{
		...
		if($comptidA!=0)
		{
			$resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.id_u=:idRec AND u.id_u=:idRec AND r.codereceptio LIKE \'CSTPR%\' ');
		...
		if($comptidM!=0)
		{
			$resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.id_u=:idRec AND u.id_u=:idRec AND r.codereceptio LIKE \'CSTPR%\' ');
	...
	?>
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneM->codereceptio;?></td>
					<td><?php echo $ligneM->nom_u.' '.$ligneM->prenom_u;?></td>
					<td>
					<?php
					$getcodeCash=$connexion->prepare('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND c.codecashier=:codeR');
					
					$getcodeCash->execute(array(
					'codeR'=>$ligneM->codereceptio	
					))or die( print_r($connexion->errorInfo()));
				
					$getcodeCash->setFetchMode(PDO::FETCH_OBJ);

					$comptidCash=$getcodeCash->rowCount();
					...
					<td><?php echo $ligneM->dateaffectationreceptio;?></td>
					...
					?>
					<td>
						<a href="utilisateurs.php?iduti=<?php echo $ligneM->id_u?>&updaterecep=<?php echo $ligneM->codeC;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
					</td>

					<td>
						<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codereceptio;?>&updaterecep=ok&divRec=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
					...
					<td></td>
					<td>
						<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codereceptio;?>$updaterecep=ok&divRec=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneA->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneA->codereceptio;?></td>
					...
					<td><?php echo $ligneA->dateaffectationreceptio;?></td>

					<td>
						<a href="report.php?rec=<?php echo $ligneA->id_u;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneA->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
		...
		<a href="receptionistes1.php?iduser=<?php echo $id;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show Receptionistes" class="btn-large">Show Receptionistes</a>
	<?php
	}
	?>
	...
	try
	{
		...
		// Numero de page (1 par défaut)
			...
			/*-----------Requête pour coordinateur-----------*/
		$resultats=$connexion->query('SELECT * FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio LIKE \'CSTPR%\' ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
			//echo $count=$resultats->rowCount();
		...
		?>
			<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
				<td><?php echo $ligne->codereceptio;?></td>
				...
				<td><?php echo $ligne->dateaffectationreceptio;?></td>
				...
				?>
				<td>
					<a href="utilisateurs.php?iduti=<?php echo $ligne->id_u?>&updaterecep=<?php echo $ligne->codeC;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
				</td>

				<td>
					<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codereceptio;?>&updaterecep=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn" title="Actif"/></a>
				</td>
				<?php
				}else{
				...
				?>
					<td></td>
					<td>
						<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codereceptio;?>&updaterecep=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn" title="Passif"/></a>
				...
				?>
						<td>
							<a href="report.php?rec=<?php echo $ligne->id_u;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
						</td>
				<?php
						}
					}
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio LIKE \'CSTPR%\' ORDER BY u.nom_u');
