...
<?php
		...
		$userinfo = '<table style="width:100%;">
			...
			{
				$userinfo .= 'Organisation: <span style="font-weight:bold">';
		...
		$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge_hosp ms, patients_hosp p WHERE p.numero=:num AND p.numero=ms.numero AND ms.numero=:num AND ms.id_hospSurge=:idhosp AND p.id_hosp=ms.id_hospSurge AND ms.datehosp!="0000-00-00" AND ms.id_factureMedSurge!="" ORDER BY ms.datehosp');
	...
	<table style="width:100%; margin-bottom:-5px">
		<tr>
			...
			<td>
				<form method="post" action="formModifierBillHosp.php?num=<?php echo $_GET['num'];?>&numbill=<?php echo $numbill;?>&idhosp=<?php echo $_GET['idhosp'];?>&datehosp=<?php echo $datehosp;?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?>&idassu=<?php echo $_GET['idassu'];?>&nomassurance=<?php echo $nomassurance;?>&billpercent=<?php echo $bill;?>&manager=<?php echo $_SESSION['codeC'];?>&datefacture=<?php echo $_GET['datefacture'];?>" enctype="multipart/form-data" class="buttonBill">
			...
			<td class="buttonBill">
				<a href="facturesedit.php?manager=<?php echo $_SESSION['codeC'];?>&numbill=<?php echo $numbill;?>&deletebillhosp=ok&idhosp=<?php echo $_GET['idhosp'];?>" class="btn" title="Supprimer la facture n° <?php echo $numbill;?>" style="margin-left:10px;"><i class="fa fa-trash fa-1x fa-fw"></i></a>
			</td>
		</tr>
	</table>
 ...
 -->
			...
			if($comptMedInf != 0)
			{
				...
				<table class="printPreview" cellspacing="0" style="margin:auto auto 5px;">
					<thead>
						<tr>
							<th style="width:20%" colspan=12>Nursing Care</th>
						</tr>
						<tr>
							<th style="width:8%;background:rgba(0, 0, 0, 0.05)" class="buttonBill">Date</th>
							<th style="width:15%;background:rgba(0, 0, 0, 0.05);text-align:left;" class="buttonBill">Name</th>
			...
			if($comptMedLabo != 0)
			{
				...
				<table class="printPreview" cellspacing="0" style="margin:auto auto 5px;">
					<thead>
						<tr>
							<th style="width:20%" colspan=12>Labs</th>
						</tr>
						<tr>
							<th style="width:8%;background:rgba(0, 0, 0, 0.05)" class="buttonBill">Date</th>
							<th style="width:15%;background:rgba(0, 0, 0, 0.05);text-align:left;" class="buttonBill">Name</th>
...
.close{
	...
	?>
		...
		<?php 
			...
			$CheckEdi->execute(array('numbill'=>$numbill));
