...
?>
		...
		<table cellpadding=3 style="background:#fff; margin:auto auto 10px auto; padding: 10px; width:100%;">
			...
			<tr>
				...
				<td style="text-align:left;">
					<h2 style="font-size:150%; font-weight:600;"><?php echo $stringResult;?> Doctor <?php if(isset($_GET['hospiAssuRepo'])){echo "Hospital Report";}else{echo "Clinic Report";} ?> #<?php echo $sn;?></h2>
				</td>
				
				<td style="text-align:right">
					
					<form method="post" action="DocSummaryRepo.php?med=<?php echo $_GET['med'];?>&dailydategnl=<?php echo $_GET['dailydategnl'];?><?php if(isset($_GET['clinicAssuRepo'])){echo '&clinicAssuRepo=ok';}if(isset($_GET['gnlMedAssuReport'])){echo '&gnlMedAssuReport=ok';}if(isset($_GET['hospiAssuRepo'])){echo '&hospiAssuRepo=ok';}?><?php if(isset($_GET['selectPersoMedicAssu'])){echo'&selectPersoMedicAssu=ok';} ?>&stringResult=<?php echo $_GET['stringResult'];?>&createReportPdf=ok&createRN=1" enctype="multipart/form-data" class="buttonBill">
	...
	<table style="width:50%; margin-top:20px;font-size:15px;" align="center">		
		...
		if (isset($_GET['clinicAssuRepo'])) {
			if(isset($_GET['gnlMedAssuReport']))
			{
		?>
			<div id="2010">
				...
				//echo $comptConsult;
				//echo 'SELECT *FROM consultations WHERE id_uM=:med AND id_factureConsult IS NOT NULL AND done=1 '.$dailydategnl;
			...
			<?php
				...
				{				
					...
					echo '<script text="text/javascript">document.location.href="DocSummaryRepo.php?med='.$_GET['med'].'&dailydategnl='.$_GET['dailydategnl'].'&clinicAssuRepo='.$_GET['clinicAssuRepo'].'&gnlMedAssuReport='.$_GET['gnlMedAssuReport'].'&stringResult='.$_GET['stringResult'].'&selectPersoMedicAssu=ok&createReportPdf=ok&createRN=0"</script>';
				}
			}
		}
	...
	if(isset($_GET['hospiAssuRepo']))
		...
		?>
			<div id="2010">
				...
				$resultConsult=$connexion->query('SELECT *FROM patients_hosp ph WHERE ph.statusPaHosp=0  '.$dailydategnl.'');	
			...
			<?php
				...
				{				
					...
					echo '<script text="text/javascript">document.location.href="DocSummaryRepo.php?med='.$_GET['med'].'&dailydategnl='.$_GET['dailydategnl'].'&hospiAssuRepo='.$_GET['hospiAssuRepo'].'&gnlMedAssuReport='.$_GET['gnlMedAssuReport'].'&stringResult='.$_GET['stringResult'].'&selectPersoMedicAssu=ok&createReportPdf=ok&createRN=0"</script>';
				}
			}
		}
	?>
