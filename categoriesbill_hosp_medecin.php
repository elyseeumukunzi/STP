...
if(isset($_GET['deleteMedConsu']))
{
	...
	echo '<script type="text/javascript">document.location.href="categoriesbill_hosp_medecin.php?inf='.$_GET['inf'].'&num='.$_GET['num'].'&idhosp='.$_GET['idhosp'].'&id_uM='.$_GET['id_uM'].'&datehosp='.$_GET['datehosp'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&numroom='.$_GET['numroom'].'&numlit='.$_GET['numlit'].'&'.$previewback.'";</script>';
...
if(isset($_GET['finishbtn']))
{
	...
	if (isset($_POST['saveHistory'])) {
		...
		$InsertHistory = $connexion->prepare("INSERT INTO clinical_history(history,numero,idHosp,id_uM,hist_date) VALUES(:history,:numero,:idHosp,:id_uM,:hist_date)");
		...
		echo '<script type="text/javascript">document.location.href="categoriesbill_hosp_medecin.php?num='.$_GET['num'].'&idhosp='.$_GET['idhosp'].'&id_uM='.$_GET['id_uM'].'&datehosp='.$_GET['datehosp'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&numroom='.$_GET['numroom'].'&numlit='.$_GET['numlit'].'&back=ok";</script>';
	}
...
<head>
	<meta charset="utf-8"/>
	<title><?php echo 'TRAITEMENT PATIENT'; ?></title>
...
function controlFormCategoBill(theForm){
	...
	if (rapportQuantityConsu != "" || rapportQuantityInf != "" || rapportQuantitySurge != "" || rapportQuantityLab != "" || rapportQuantityRad != "" || rapportQuantityKine != "" || rapportQuantityConsom != "" || rapportQuantityMedoc != "" || rapportQuantityOrtho != "") {
...
if($connect==true AND ($comptidI!=0 OR $comptidD!=0 OR $comptidC!=0) AND isset($_GET['num']))
{
	if($status==1)
	{
...
if($comptidI!=0 OR $comptidD!=0 OR $comptidC!=0)
{
?>
...
?>
	...
	<hr>
		<button onclick="Clinical('show');" class="btn btn-large-inversed" id="show" style="position:relative;left: 43%;
		margin-top: -5px;"><i class="fa fa-eye"></i> <?php echo "Patient Clinical History";?>
		</button>
		<button onclick="Clinical('hide');" class="btn btn-large-inversed" id="hide" style="position:relative;left: 43%;
		margin-top: -5px;display: none;"><i class="fa fa-eye-slash"></i> <?php echo "Hide Clinical history Panel";?>
		</button>
		<br/><br/>
		<div style="height: auto;width: 70%;border: 1px solid #A00000;box-shadow: 0 0 3px #B20000;border-radius: 9px;background: #ddd;position: relative;left: 16%;font-family:ubuntu;"> 
			<form method="POST" style="display: none;" id="cliniHis">
				<h4 style="border-bottom: 2px solid #A00000;padding: 10px 10px;background: black;color: white;border-top-left-radius: 9px;border-top-right-radius: 9px;text-align: center;">Patient History Recording</h4>
				<br>
				<textarea name="ClinicalHistory" style="height: 150px;width: 70%;"></textarea>
			<button class="btn btn-large" name="saveHistory">Save History</button>
			</form>
		</div>
		<br/>
		<br>
	
	<form method="post" action="addpresta_hosp_medecin.php?num=<?php echo $_GET['num'];?>&idassu=<?php echo $_GET['idassu'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idhosp=<?php echo $_GET['idhosp'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?>" onsubmit="return controlFormCategoBillHosp(this)" enctype="multipart/form-data">
		...
		<table align="center">
			...
			$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge_hosp ms WHERE ms.soinsfait=0 AND ms.numero=:num AND ms.id_factureMedSurge="" AND ms.id_hospSurge=:idhosp ORDER BY ms.id_medsurge');		
			...
			?>
				...
				if(!isset($_GET['previewprint']))
				{
					$id_uM=$_SESSION['id'];
				?>
					<a style="padding:10px 40px;" href="<?php echo 'categoriesbill_hosp_medecin.php?num='.$_GET['num'].'&idhosp='.$_GET['idhosp'].'&id_uM='.$id_uM.'&datehosp='.$_GET['datehosp'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&numroom='.$_GET['numroom'].'&numlit='.$_GET['numlit'].'&previewprint=ok'?>" class="btn-large-inversed" name="showpreviewbtn" id="showpreviewbtn"><?php echo 'Edit Profile'; ?></a>
				<?php
				}else{
					if(isset($_GET['previewprint']))
					{
		...
		<div id="divCatego" style="margin:40px auto 0; text-align:center">
			...
			<table align="center" style="display:inline;" id="catego">
				<tr>	
					...
					<td>
						<span id="services" class="btn-large" onclick="ShowMore('services')"><?php echo "Autres Services";?></span>
					</td>
				</tr>
			...
			<table id="showService" align="center" style="margin:20px auto; display:none;width:100%;background:#ddd;">
				<tr>
					<td style="padding:5px;text-align:center;">
						...
						$resultatsCategoPrestaConsu->setFetchMode(PDO::FETCH_OBJ);
						...
						while($ligneCategoPrestaConsu=$resultatsCategoPrestaConsu->fetch())
						{
							...
							$resultatsPrestaConsu=$connexion->query('SELECT *FROM '.$presta_assu.' p, categopresta_ins cp WHERE cp.id_categopresta=p.id_categopresta AND (cp.id_categopresta=5 OR cp.id_categopresta=6 OR cp.id_categopresta=7 OR cp.id_categopresta=8 OR cp.id_categopresta=19 OR cp.id_categopresta=20)  AND p.statupresta!=0 AND p.id_categopresta='.$ligneCategoPrestaConsu->id_categopresta.'  ORDER BY p.nompresta');
			...
			<table id="surge" align="center" style="margin:20px auto; width:100%;background:#ddd;">
				<tr>
					<td style="padding:5px;text-align:center;">
						...
						$resultatsPrestaSurge=$connexion->query('SELECT *FROM '.$presta_assu.' p, categopresta_ins cp WHERE cp.id_categopresta=p.id_categopresta AND p.id_categopresta=4 AND p.statupresta!=0 ORDER BY p.nompresta ASC');
	...
	?>
		...
		?>
			...
			<form method="post" action="newTest_medecin.php?num=<?php echo $_GET['num'];?>&idassu=<?php echo $_GET['idassu'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['id_consuHosp'])){ echo '&id_consuHosp='.$_GET['id_consuHosp'];}?><?php if(isset($_GET['datefacture'])){ echo '&datefacture='.$_GET['datefacture'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" <?php if(!isset($_GET['infShow'])){ echo 'onsubmit="return controlFormCategoBill(this)"';}?> enctype="multipart/form-data">
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableSurge"> 
					<thead> 
						<tr>
							...
							<th style="width:10%"><?php echo 'Doctor';?></th>
					...
					?>
						<tr style="text-align:center;">
							...
							<td>						
								<span type="submit" id="qteSurgeMoins<?php echo $l;?>" name="qteSurgeMoins<?php echo $l;?>" class="qteSurgeMoins btn" onclick="MoinsQteSurge(<?php echo $l;?>)"/>-</span>
								
								<input type="text" id="quantitySurge<?php echo $l;?>" name="quantitySurge[]" class="quantitySurge" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedSurge->qteSurge;?>" readonly="readonly"/>
							...
							<td>
								...
								{
									$fullnameMed=$ligneNomMed->full_name;
								}
							...
							if($ligneMedSurge->id_prestationSurge==0 AND $ligneMedSurge->prixautreSurge==0)
							{
								...
								echo $ligneMedSurge->autreSurge.'<img src="icones/s_warn.png" style="width:20px;height:20px;margin-left:20px;"/>
									</td>
							...
							<td style="display:none">
								<input type="text" id="percentSurge" name="percentSurge[]" class="percentSurge" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedSurge=<?php echo $ligneMedSurge->id_medsurge;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>

							</td>
						</tr>
						
						<tr>
							<td colspan=6 style="border:0 20px 0 0">
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableInf"> 
					...
					?>
						...
						<td>
							...
							<td>
								<span type="submit" id="qteInfMoins<?php echo $f;?>" name="qteInfMoins<?php echo $f;?>" class="qteInfMoins btn" onclick="MoinsQteInf(<?php echo $f;?>)"/>-</span>
								
								<input type="text" id="quantityInf<?php echo $f;?>" name="quantityInf[]" class="quantityInf" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedInf->qteInf;?>" readonly="readonly"/>
...
<input type="hidden" name="idprestaInf[]" style="width:100px;display:none;" id="idprestaInf" value="0"/>
							...
							<td style="display:none;">
								<input type="text" id="percentInf" name="percentInf[]" class="percentInf" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
								<input type="hidden" id="idmedInf" name="idmedInf[]" style="width:50px; text-align:center" value="<?php echo $ligneMedInf->id_medinf;?>"/>
							
							</td>
							
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedInf=<?php echo $ligneMedInf->id_medinf;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableLab"> 
					<thead> 
						<tr>
							...
							<th style="width:15%;display:none;"><?php echo getString(38);?></th>
							<th style="width:auto"><?php echo getString(21);?></th>
							<th style="width:auto"><?php echo 'Doctor';?></th>
							<th style="width:auto"><?php echo 'Action';?></th>
						</tr> 
					</thead> 
					...
					?>
						<tr style="text-align:center;">
							...
							<td>
								<span type="submit" id="qteLabMoins<?php echo $g;?>" name="qteLabMoins<?php echo $g;?>" class="qteLabMoins btn" onclick="MoinsQteLab(<?php echo $g;?>)"/>-</span>
								
								<input type="text" id="quantityLab<?php echo $g;?>" name="quantityLab[]" class="quantityLab" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedLabo->qteLab;?>" readonly="readonly"/>
							...
							<td style="display:none;">
								<input type="text" id="percentLab" name="percentLab[]" class="percentLab" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
								<input type="hidden" id="idmedLab" name="idmedLab[]" style="width:50px; text-align:center" value="<?php echo $ligneMedLabo->id_medlabo;?>"/>
							</td>
							
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedLabo=<?php echo $ligneMedLabo->id_medlabo;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
		...
		<?php
			...
			{
		?>	
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableRad"> 
					<thead> 
						<tr>
							<th style="width:35%">Date</th>
							<th style="width:25%"><?php echo 'Radiology tests';?></th>
							<th style="display:none;"><?php echo getString(145);?></th>
							<th style="width:25%"><?php echo getString(215);?></th>
							<th style="width:15%;display:none;"><?php echo getString(38);?></th>
							<th style="width:auto"><?php echo getString(21);?></th>
					...
					?>
						<tr style="text-align:center;">
							...
							<td>
								<span type="submit" id="qteRadMoins<?php echo $h;?>" name="qteRadMoins<?php echo $h;?>" class="qteRadMoins btn" onclick="MoinsQteRad(<?php echo $h;?>)"/>-</span>
								
								<input type="text" id="quantityRad<?php echo $h;?>" name="quantityRad[]" class="quantityRad" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedRadio->qteRad;?>" readonly="readonly"/>
							...
							<td style="display:none;">
								<input type="text" id="percentRad" name="percentRad[]" class="percentRad" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
								<input type="hidden" id="idmedRad" name="idmedRad[]" style="width:50px; text-align:center" value="<?php echo $ligneMedRadio->id_medradio;?>"/>
							</td>
							
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedRadio=<?php echo $ligneMedRadio->id_medradio;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableConsom"> 
					...
					?>
						<tr style="text-align:center;">
							...
							<td>
								<span type="submit" id="qteConsomMoins<?php echo $j;?>" name="qteConsomMoins<?php echo $j;?>" class="qteConsomMoins btn" onclick="MoinsQteConsom(<?php echo $j;?>)"/>-</span>
								
								<input type="text" id="quantityConsom<?php echo $j;?>" name="quantityConsom[]" class="quantityConsom" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedConsom->qteConsom;?>" readonly="readonly"/>
							...
							<td>
								...
								$getNomMed=$connexion->prepare('SELECT *FROM utilisateurs u, medecins m WHERE m.id_u=:operation AND u.id_u=m.id_u');
								$getNomMed->execute(array(
								'operation'=>$ligneMedConsom->id_uM	
								));
								$getNomMed->setFetchMode(PDO::FETCH_OBJ);
							...
							<td style="display:none;">
								<input type="text" id="percentConsom" name="percentConsom[]" class="percentConsom" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedConsom=<?php echo $ligneMedConsom->id_medconsom;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableMedoc"> 
					...
					?>
						<tr style="text-align:center;">
							...
							<td>						
								<span type="submit" id="qteMedocMoins<?php echo $k;?>" name="qteMedocMoins<?php echo $k;?>" class="qteMedocMoins btn" onclick="MoinsQteMedoc(<?php echo $k;?>)"/>-</span>	
								<input type="text" id="quantityMedoc<?php echo $k;?>" name="quantityMedoc[]" class="quantityMedoc" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedMedoc->qteMedoc;?>" readonly="readonly"/>
							...
							if($ligneMedMedoc->id_prestationMedoc==0 AND $ligneMedMedoc->prixautreMedoc==0)
							{
								...
								if($ligneMedMedoc->id_prestationMedoc==0 AND $ligneMedMedoc->prixautreMedoc!=0)
								{
									...
									echo '<td>
									<input type="text" name="prixprestaMedoc[]" style="width:100px;" id="prixprestaMedoc" class="prixprestaMedoc" value="'.$ligneMedMedoc->prixautreMedoc.'" onchange="ShowSaveMedoc('.$k.')"/>';
								}
							...
							<td style="display:none;">
								<input type="text" id="percentMedoc" name="percentMedoc[]" class="percentMedoc" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedMedoc=<?php echo $ligneMedMedoc->id_medmedoc;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
					...
					?>
						<tr style="text-align:center;">
							...
							for($i=1;$i<=$assuCount;$i++)
							{
								...
								?>							
							<td>						
								<span type="submit" id="qteKineMoins<?php echo $o;?>" name="qteKineMoins<?php echo $o;?>" class="qteKineMoins btn" onclick="MoinsQteKine(<?php echo $o;?>)"/>-</span>	
								<input type="text" id="quantityKine<?php echo $o;?>" name="quantityKine[]" class="quantityKine" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedKine->qteKine;?>" readonly="readonly"/>
							...
							<td style="display:none;">
								<input type="text" id="percentKine" name="percentKine[]" class="percentKine" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedKine=<?php echo $ligneMedKine->id_medkine;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
		...
		?>	
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableOrtho"> 
					<thead> 
						<tr>
							<th style="width:35%">Date</th>
							<th style="width:25%"><?php echo getString(273);?></th>
							<th style="display:none;"><?php echo getString(145);?></th>
							<th style="width:25%"><?php echo getString(215);?></th>
							<th style="width:15%;display:none;"><?php echo getString(38);?></th>
							<th style="width:auto"><?php echo getString(21);?></th>
					...
					?>
						<tr style="text-align:center;">
							...
							<td>						
								<span type="submit" id="qteOrthoMoins<?php echo $n;?>" name="qteOrthoMoins<?php echo $n;?>" class="qteOrthoMoins btn" onclick="MoinsQteOrtho(<?php echo $n;?>)"/>-</span>
								
								<input type="text" id="quantityOrtho<?php echo $n;?>" name="quantityOrtho[]" class="quantityOrtho" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedOrtho->qteOrtho;?>" readonly="readonly"/>
							...
							<td style="display:none;">								
								<input type="text" name="prixprestaOrtho[]" style="width:100px; text-align:center" id="prixprestaOrtho" class="prixprestaOrtho" value="<?php if($lignePresta->prixpresta!=-1){ echo $lignePresta->prixpresta; }else{ echo "";}?>" onchange="ShowSaveOrtho(<?php echo $n;?>)"/>
							...
							if($ligneMedOrtho->id_prestationOrtho==0 AND $ligneMedOrtho->prixautreOrtho==0)
							{
								...
								echo $ligneMedOrtho->autreOrtho.'<img src="icones/s_warn.png" style="width:20px;height:20px;margin-left:20px;"/></td>
									<td>
										<input type="text" name="prixprestaOrtho[]" style="width:100px; text-align:center" id="prixprestaOrtho" class="prixprestaOrtho" value="" placeholder="Tarrif ici..." onchange="ShowSaveOrtho('.$n.')"/>
								...
								if($ligneMedOrtho->id_prestationOrtho==0 AND $ligneMedOrtho->prixautreOrtho!=0)
								{
									...
									echo '<td>
									<input type="text" name="prixprestaOrtho[]" style="width:100px;" id="prixprestaOrtho" class="prixprestaOrtho" value="'.$ligneMedOrtho->prixautreOrtho.'" onchange="ShowSaveOrtho('.$n.')"/>';
								}
							...
							<td style="display:none;">
								<input type="text" id="percentOrtho" name="percentOrtho[]" class="percentOrtho" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedOrtho=<?php echo $ligneMedOrtho->id_medortho;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; margin-top:10px;" id="tableServ"> 
					...
					?>
						<tr style="text-align:center;">
							...
							<td>
								<span type="submit" id="qteConsuMoins<?php echo $e;?>" name="qteConsuMoins<?php echo $e;?>" class="qteConsuMoins btn" onclick="MoinsQteConsu(<?php echo $e;?>)"/>-</span>
								
								<input type="text" id="quantityConsu<?php echo $e;?>" name="quantityConsu[]" class="quantityConsu" style="width:30px;margin-left:0px;background:#F8F8F8;" value="<?php echo $ligneMedConsult->qteConsu;?>" readonly="readonly/>
							...
							<td>
								...
								$getNomMed=$connexion->prepare('SELECT *FROM utilisateurs u, medecins m WHERE m.id_u=:operation AND u.id_u=m.id_u');
								$getNomMed->execute(array(
								'operation'=>$ligneMedConsult->id_uM	
								));
								$getNomMed->setFetchMode(PDO::FETCH_OBJ);
							...
							<td style="display:none;">
								<input type="text" name="percentConsu[]" class="percentConsu" id="percentConsu<?php echo $e;?>" style="width:30px; text-align:center;<?php if($comptidD!=0){ echo 'background:#F8F8F8;';}?>" value="<?php echo $bill;?>" <?php if($comptidD!=0){ echo 'readonly="readonly"';}?>/> %
							...
							<td>
								<a href="categoriesbill_hosp_medecin.php?deleteMedConsu=<?php echo $ligneMedConsult->id_medconsu;?>&num=<?php echo $_GET['num'];?>&idhosp=<?php echo $_GET['idhosp'];?>&id_uM=<?php echo $_GET['id_uM'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?><?php if(isset($_GET['datefacture'])){ echo $_GET['datefacture'];}?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
...
<div>
	...
	function Clinical(fld){
		...
		document.getElementById('DischargeMotif').style.display='none';
		}	
		...
		if(fld == 'Discharge'){
		document.getElementById('DischargeMotif').style.display='inline';
		document.getElementById('cliniHis').style.display='none';
		document.getElementById('show').style.display='inline';
		document.getElementById('hide').style.display='none';
		}
