			...
			<table class="printPreview" style="margin-top:2px;">
				...
				if(isset($_POST['vouchernum']))
				{
					...
					'vouchernum'=>$vouchernum
						...
						$resultMedPsy=$connexion->prepare('SELECT *FROM med_psy mp, patients p WHERE p.numero=:num AND p.numero=mp.numero AND mp.numero=:num AND mp.id_consuPSy=:idconsu AND (mp.id_factureMedPsy=0 OR mp.id_factureMedPsy IS NULL) ORDER BY mp.id_medpsy');
...
*/
					...
					*/
					...
					<tr>
						...
						?>
						<td style="text-align:left; font-size: 110%; font-weight: bold;">
							<table class="printPreview" cellspacing="0" style="margin-top:5px;border:none;" align="left">
								<tr>
									<td style="font-size: 110%; font-weight: bold;border:none;" colspan=2>
								<?php
									echo $TotalGnlPatientPrice;
								?><span style="font-size:70%; font-weight:normal;">Rwf</span>
									</td>
...
<?php
		...
		$resultMedPsy=$connexion->prepare('SELECT *FROM med_psy mo, patients p WHERE p.numero=:num AND p.numero=mo.numero AND mo.numero=:num AND mo.id_consuPSy=:idconsu AND (mo.id_factureMedPsy=0 OR mo.id_factureMedPsy IS NULL) ORDER BY mo.id_medpsy');
		$resultMedPsy->execute(array(
		'num'=>$numPa,
		'idconsu'=>$_GET['idconsu']
		));
