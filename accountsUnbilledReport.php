...
<head>
    <meta charset="utf-8"/>
    <title><?php echo 'Cashier UnBilled  Report#'; ?></title>
            ...
            <table cellpadding=3 style="background:#fff; margin:auto auto 10px auto; padding: 10px; width:100%;">
                <tr>
                    ...
                    <td style="text-align:left;">
                        <h2 style="font-size:150%; font-weight:600;"><?php echo $stringResult;?> | <?php if(isset($_GET['divGnlUnbilledReport'])){ echo ' UnBilled Cashier Report #';}?></h2>
                    </td>

                    <td style="text-align:right">
                        <!-- num=<?php echo $_GET['cash']; ?>&id_uC=<?php echo $_GET['id_uC']; ?> -->

                        <form method="post" action="accountsUnbilledReport.php?&docVisit=<?php echo $_GET['docVisit'];?>&dailydateperso=<?php echo $_GET['dailydateperso'];?>&stringResult=<?php echo $_GET['stringResult'];?>&clinic=<?php echo $_GET['clinic'];?>&divGnlUnbilledReport=ok&UnBilledbill=ok&createReportPdf=ok" enctype="multipart/form-data" class="buttonBill">
	...
	<table style="width:50%; margin-top:20px;font-size:15px;" align="center">		
		...
		<tr>
            ...
            if(isset($_GET['divGnlUnbilledReport']))
            {
                ...
                $resultatsC = $connexion->query("SELECT *FROM consultations c WHERE c.id_factureConsult IS NULL ".$dailydategnl."");
                ...
                ?>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th style="width:10%;text-align: center;">Cashier Name</th>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <?php
                                    $selectCashInfo = $connexion->prepare("SELECT * FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND c.id_u=:id_u");
                                    $selectCashInfo->execute(array(
                                        'id_u'=>$getthem->id_uR
                                    ));
                                    $selectCashInfo->setFetchMode(PDO::FETCH_OBJ);
                                    $ligneCashInfo = $selectCashInfo->fetch();
                                    echo $ligneCashInfo->full_name;
                                ?>
                            </td>
                            ...
                            <td style="text-align: center;">
                                <?php 
                                    if($comptConsult!=0 OR $comptMedInf!=0 OR $comptMedLabo!=0 OR $comptMedConsom!=0 OR $comptMedMedoc!=0 OR $comptMedRadio!=0 OR $comptMedSurge!=0 OR $comptMedConsult!=0 ){
                                        ?> 
                                        <a href="accountsUnBilledView.php?audit=<?php echo $_SESSION['id'];?>&clinic=<?php echo $_GET['clinic'];?>&docVisit=<?php echo $_GET['docVisit'];?>&cash=<?php echo $getthem->id_uR; ?>&stringResult=<?php echo $_GET['stringResult']; ?>&dailydateperso=<?php echo $_GET['dailydateperso'];?>&divGnlUnbilledReport=ok&cashierUnBilledbill=ok&paVisit=ok&idconsu=<?php echo $getthem->id_consu;?>&num=<?php echo $getthem->numero;?>" class="btn">View Bill <i class='fa fa-info-circle flashing' style="font-size: 20px;padding: 10px 13px;color: red;border-radius: 50px;" class="flashing"></i></a>
