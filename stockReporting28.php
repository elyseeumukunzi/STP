...
?>
	...
	?>
		<div id="divPersoMedicReport">
			...
			<?php
			$resultStockIn=$connexion->query('SELECT *  FROM stockin_history sthI,stockin sti WHERE sthI.status = '.$statusCate.' AND sti.sid=sthI.sid '.$dailydateperso.'');		
			...
			<?php
			$resultStockOut=$connexion->query('SELECT *  FROM stockout_history sth,stockin sti WHERE sth.status = '.$statusCate.' AND sti.sid=sth.sid '.$dailydateperso.'');		
		...
		if(isset($_GET['ExpiredDrugs']))
			...
			$resultStockOut=$connexion->query('SELECT *  FROM stockout_history sth,stockin sti WHERE sth.status = '.$statusCate.' AND sti.quantity!=0   AND sti.sid=sth.sid '.$dailydateperso.'  GROUP BY sti.sid ');		
