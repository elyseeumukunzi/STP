			...
			if($mois<0)
			{
				...
				// echo $mois= date('m')-$month;
					...
					<td style="text-align:center;">
						...
						<?php
                        $balanceBalance = ($balanceCCO - $balance) + ($prixconsultpatient + $prixconsultinsu);
					...
					if($comptMedConsu!=0)
					{
						while($ligneMedConsu=$resultMedConsu->fetch())
						{
							if($ligneMedConsu->prixprestationConsu!=0 AND $ligneMedConsu->prixrembouConsu!=0)
							{
								...
								$prixconsu=($ligneMedConsu->prixprestationConsu * $ligneMedConsu->qteConsu) - $prixPrestaRembou;
								$prixconsuCCO=($ligneMedConsu->prixprestationConsuCCO * $ligneMedConsu->qteConsu) - $prixPrestaRembou;

							}else{
								$prixconsu=$ligneMedConsu->prixprestationConsu * $ligneMedConsu->qteConsu;
								$prixconsuCCO=$ligneMedConsu->prixprestationConsuCCO * $ligneMedConsu->qteConsu;
					...
					if($comptMedAutreConsu!=0)
					{
						while($ligneMedAutreConsu=$resultMedAutreConsu->fetch())
						{
							if($ligneMedAutreConsu->prixautreConsu!=0 AND $ligneMedAutreConsu->prixrembouConsu!=0)
							{
								...
								$prixconsu=($ligneMedAutreConsu->prixautreConsu * $ligneMedAutreConsu->qteConsu) - $prixPrestaRembou;
								$prixconsuCCO=($ligneMedAutreConsu->prixautreConsuCCO * $ligneMedAutreConsu->qteConsu) - $prixPrestaRembou;

							}else{
								$prixconsu=$ligneMedAutreConsu->prixautreConsu * $ligneMedAutreConsu->qteConsu;
								$prixconsuCCO=$ligneMedAutreConsu->prixautreConsuCCO * $ligneMedAutreConsu->qteConsu;
					...
					if($comptMedInf!=0)
					{
						while($ligneMedInf=$resultMedInf->fetch())
						{
							if($ligneMedInf->prixprestation!=0 AND $ligneMedInf->prixrembouInf!=0)
							{
								...
								$prixinf=($ligneMedInf->prixprestation * $ligneMedInf->qteInf) - $prixPrestaRembou;
					...
					if($comptMedAutreInf!=0)
					{
						while($ligneMedAutreInf=$resultMedAutreInf->fetch())
						{
							if($ligneMedAutreInf->prixautrePrestaM!=0 AND $ligneMedAutreInf->prixrembouInf!=0)
							{
								...
								$prixinf=($ligneMedAutreInf->prixautrePrestaM * $ligneMedAutreInf->qteInf) - $prixPrestaRembou;
							
							}else{
								$prixinf=$ligneMedAutreInf->prixautrePrestaM * $ligneMedAutreInf->qteInf;
					...
					if($comptMedLabo!=0)
					{
						while($ligneMedLabo=$resultMedLabo->fetch())
						{
							if($ligneMedLabo->prixprestationExa!=0 AND $ligneMedLabo->prixrembouLabo!=0)
							{
								...
								$prixlabo=($ligneMedLabo->prixprestationExa * $ligneMedLabo->qteLab) - $prixPrestaRembou;
					...
					if($comptMedAutreLabo!=0)
					{
						while($ligneMedAutreLabo=$resultMedAutreLabo->fetch())
						{
							if($ligneMedAutreLabo->prixautreExamen!=0 AND $ligneMedAutreLabo->prixrembouLabo!=0)
							{
								...
								$prixlabo=($ligneMedAutreLabo->prixautreExamen * $ligneMedAutreLabo->qteLab) - $prixPrestaRembou;
							
							}else{
								$prixlabo=$ligneMedAutreLabo->prixautreExamen * $ligneMedAutreLabo->qteLab;
					...
					if($comptMedRadio!=0)
					{
						while($ligneMedRadio=$resultMedRadio->fetch())
						{
							if($ligneMedRadio->prixprestationRadio!=0 AND $ligneMedRadio->prixrembouRadio!=0)
							{
								...
								$prixradio=($ligneMedRadio->prixprestationRadio * $ligneMedRadio->qteRad) - $prixPrestaRembou;
					...
					if($comptMedAutreRadio!=0)
					{
						while($ligneMedAutreRadio=$resultMedAutreRadio->fetch())//on recupere la liste des �l�ments
						{
							if($ligneMedAutreRadio->prixautreRadio!=0 AND $ligneMedAutreRadio->prixrembouRadio!=0)
							{
								...
								$prixradio=($ligneMedAutreRadio->prixautreRadio * $ligneMedAutreRadio->qteRad) - $prixPrestaRembou;
							
							}else{
								$prixradio=$ligneMedAutreRadio->prixautreRadio * $ligneMedAutreRadio->qteRad;
