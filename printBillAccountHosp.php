...
$checkIdBill=$connexion->prepare('SELECT *FROM patients_hosp ph WHERE ph.id_hosp=:id_hosp ORDER BY ph.id_factureHosp LIMIT 1');
...
if($connected==true AND isset($_SESSION['codeAcc']))
{
	...
	<table style="width:100%">
		...
		$resultatHosp=$connexion->prepare('SELECT *FROM patients_hosp ph WHERE ph.id_hosp=:id_hosp');
		...
		$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge_hosp ms, patients_hosp p WHERE p.numero=:num AND p.numero=ms.numero AND ms.numero=:num AND ms.id_hospSurge=:id_hosp AND p.id_hosp=ms.id_hospSurge AND ms.datehosp!="0000-00-00" AND ms.id_factureMedSurge!=""  ORDER BY ms.id_medsurge');
		...
		$resultMedInf=$connexion->prepare('SELECT *FROM med_inf_hosp mi, patients_hosp p WHERE p.numero=:num AND p.numero=mi.numero AND mi.numero=:num AND mi.id_hospInf=:id_hosp AND p.id_hosp=mi.id_hospInf AND mi.id_factureMedInf!="" ORDER BY mi.id_medinf');
		...
		$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo_hosp ml, patients_hosp p WHERE p.numero=:num AND p.numero=ml.numero AND ml.numero=:num AND ml.id_hospLabo=:id_hosp AND p.id_hosp=ml.id_hospLabo AND ml.id_factureMedLabo!="" ORDER BY ml.id_medlabo');
		...
		$resultMedRadio=$connexion->prepare('SELECT *FROM med_radio_hosp mr, patients_hosp p WHERE p.numero=:num AND p.numero=mr.numero AND mr.numero=:num AND mr.id_hospRadio=:id_hosp AND p.id_hosp=mr.id_hospRadio AND mr.id_factureMedRadio!="" ORDER BY mr.id_medradio');
		...
		$resultMedKine=$connexion->prepare('SELECT *FROM med_kine_hosp mk, patients_hosp p WHERE p.numero=:num AND p.numero=mk.numero AND mk.numero=:num AND mk.id_hospKine=:id_hosp AND p.id_hosp=mk.id_hospKine AND mk.id_factureMedKine!="" ORDER BY mk.id_medKine');
		...
		$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom_hosp mco, patients_hosp p WHERE p.numero=:num AND p.numero=mco.numero AND mco.numero=:num AND mco.id_hospConsom=:id_hosp AND p.id_hosp=mco.id_hospConsom AND mco.id_factureMedConsom!="" ORDER BY mco.id_medconsom');
		...
		$resultMedMedoc=$connexion->prepare('SELECT *FROM med_medoc_hosp mdo, patients_hosp p WHERE p.numero=:num AND p.numero=mdo.numero AND mdo.numero=:num AND mdo.id_hospMedoc=:id_hosp AND p.id_hosp=mdo.id_hospMedoc AND mdo.id_factureMedMedoc!="" ORDER BY mdo.id_medmedoc');
		...
		$resultMedOrtho=$connexion->prepare('SELECT *FROM med_ortho_hosp mo, patients_hosp p WHERE p.numero=:num AND p.numero=mo.numero AND mo.numero=:num AND mo.id_hospOrtho=:id_hosp AND p.id_hosp=mo.id_hospOrtho AND mo.id_factureMedOrtho!="" ORDER BY mo.id_medOrtho');
		...
		<table style="width:100%; margin-bottom:-5px">
			<tr>
				...
				<td class="buttonBill">
					<a href="billsaccount_hosp.php?codeAcc=<?php echo $_SESSION['codeAcc'];?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" id="cancelbtn" style="<?php if(!isset($_GET['finishbtn'])){ echo "display:inline";}else{ echo "display:none";}?>;margin:5px;">
						<button class="btn-large-inversed" style="width:150px;"><i class="fa fa-ban fa-lg fa-fw"></i> <?php echo getString(140);?></button>
					</a>
					
					<a href="billsaccount_hosp.php?codeAcc=<?php echo $_SESSION['codeAcc'];?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" id="finishbtn" style="<?php if(!isset($_GET['finishbtn'])){ echo "display:none";}?>;margin:5px;">
