		...
		<div id="divGnlBillReport" style="font-weight:normal;">
			...
			if($comptBillReport!=0)
			{
				...
				$objPHPExcel->setActiveSheetIndex(0)
								...
								->setCellValue('D9', 'N° DE LA CARTE D\'ASSURE')
								->setCellValue('E9', 'NOM & PRENOM DU MALADE')
								->setCellValue('F9', 'CONSULTATION')
								->setCellValue('G9', 'SERVICES')
								->setCellValue('H9', 'EXAMEN COMPL.')
								->setCellValue('I9', 'RADIOLOGIE')
								->setCellValue('J9', 'INFIRMERIE')
								->setCellValue('K9', 'CONSOMMABLES')
								->setCellValue('L9', 'MEDICAMENTS')
								->setCellValue('M9', 'MONTANT 100%')
								->setCellValue('N9', 'MONTANT '.$_GET['nomassu'].'');
			...
			if($mois<0)
			{
				...
				?>
					...
					$resultMedRadio->setFetchMode(PDO::FETCH_OBJ);//on veut que le r?ultat soit r?up?able sous forme d'objet
				...
				if($comptMedRadio!=0 or $comptMedAutreRadio!=0)
				{
					...
					{
						while($ligneMedAutreRadio=$resultMedAutreRadio->fetch())//on recupere la liste des ??ents
						{
				...
				if($comptMedInf!=0 or $comptMedAutreInf!=0)
				{
					...
					{
						while($ligneMedAutreInf=$resultMedAutreInf->fetch())//on recupere la liste des ??ents
						{
				...
				?>
					...
					$resultMedConsom->setFetchMode(PDO::FETCH_OBJ);//on veut que le r?ultat soit r?up?able sous forme d'objet
				...
				?>
					...
					{
						while($ligneMedAutreConsom=$resultMedAutreConsom->fetch())//on recupere la liste des ??ents
						{
				...
				?>
					...
					$resultMedMedoc->setFetchMode(PDO::FETCH_OBJ);//on veut que le r?ultat soit r?up?able sous forme d'objet
				...
				?>
					...
					{
						while($ligneMedAutreMedoc=$resultMedAutreMedoc->fetch())//on recupere la liste des ??ents
						{
				...
				$TotalGnlPrice=$TotalGnlPrice + $TotalDayPrice;
					...
					$arrayGnlBillReport[$i][5]=$TotalTypeConsu;
					
					// $arrayGnlBillReport[$i][6]=$medconsu;		
					$arrayGnlBillReport[$i][6]=$TotalMedConsu;
										
					// $arrayGnlBillReport[$i][7]=$medlabo;		
					$arrayGnlBillReport[$i][7]=$TotalMedLabo;
					
					// $arrayGnlBillReport[$i][8]=$medradio;		
					$arrayGnlBillReport[$i][8]=$TotalMedRadio;
					
					// $arrayGnlBillReport[$i][9]=$medinf;		
					$arrayGnlBillReport[$i][9]=$TotalMedInf;
					
					// $arrayGnlBillReport[$i][10]=$medconsom;		
					$arrayGnlBillReport[$i][10]=$TotalMedConsom;
					
					// $arrayGnlBillReport[$i][11]=$medmedoc;		
					$arrayGnlBillReport[$i][11]=$TotalMedMedoc;
					
					$arrayGnlBillReport[$i][12]=$TotalDayPrice;
					$arrayGnlBillReport[$i][13]=$TotalDayPriceInsu;
			...
			<?php
				...
				$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue('J'.(10+$i).'', ''.$TotalGnlMedRadio.'')
							->setCellValue('K'.(10+$i).'', ''.$TotalGnlMedConsom.'')
							->setCellValue('L'.(10+$i).'', ''.$TotalGnlMedMedoc.'')
							->setCellValue('M'.(10+$i).'', ''.$TotalGnlPrice.'')
							->setCellValue('N'.(10+$i).'', ''.$TotalGnlPriceInsu.'');
