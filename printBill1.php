...
?>
	<div style="margin: 10px auto auto; width:95%; border: 1px solid #ccc; background:#fff; padding:2px; border-radius:3px; font-size:70%;">
		...
		<tr>
			...
			<table class="printPreview" style="margin-top:2px;">			
				...
				?> 
					...
					<td style="text-align:center">
						Insurance type: <span style="font-weight:bold"><?php echo $nomassurance.' ('.$bill.'%)';?></span>
						<?php
						if($profession!="")
						{
						?>
			...
			<table class="printPreview" style="margin-top:2px;">
				...
				if(isset($_POST['vouchernum']))
				{
					...
					'vouchernum'=>$vouchernum
						...
						$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge ms, patients p WHERE p.numero=:num AND p.numero=ms.numero AND ms.numero=:num AND ms.id_consuSurge=:idconsu AND (ms.id_factureMedSurge=0 OR ms.id_factureMedSurge IS NULL) ORDER BY ms.id_medsurge');		
                        ...
                        if(isset($_POST['idprestaKine']))
                        {
                            ...
                            for($i=0;$i<sizeof($addKine);$i++)
                            {
                                ...
                                if($comptPresta!=0)
                                {
                                    $updatepercent=$connexion->query('UPDATE med_kine mk SET mk.id_assuKine='.$_GET['idassu'].',mk.insupercentKine='.$addKine[$i].',mk.prixprestationKine='.$prixmk[$i].',mk.prixprestationKineCCO='.$prixmkCCO[$i].',mk.prixautrePrestaO=0,mk.prixautrePrestaOCCO=0 WHERE mk.id_medkine='.$idmk[$i].'');
                        ...
                        $resultMedKine=$connexion->prepare('SELECT *FROM med_kine mk, patients p WHERE p.numero=:num AND p.numero=mk.numero AND mk.numero=:num AND mk.id_consuKine=:idconsu AND (mk.id_factureMedKine=0 OR mk.id_factureMedKine IS NULL) ORDER BY mk.id_medkine');
						...
						$resultMedInf=$connexion->prepare('SELECT *FROM med_inf mi, patients p WHERE p.numero=:num AND p.numero=mi.numero AND mi.numero=:num AND mi.id_consuInf=:idconsu AND (mi.id_factureMedInf=0 OR mi.id_factureMedInf IS NULL) ORDER BY mi.id_medinf');		
						...
						$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo ml, patients p WHERE p.numero=:num AND p.numero=ml.numero AND ml.numero=:num AND ml.id_consuLabo=:idconsu AND (ml.id_factureMedLabo=0 OR ml.id_factureMedLabo IS NULL) ORDER BY ml.id_medlabo');		
						...
						$resultMedRadio=$connexion->prepare('SELECT *FROM med_radio mr, patients p WHERE p.numero=:num AND p.numero=mr.numero AND mr.numero=:num AND mr.id_consuRadio=:idconsu AND (mr.id_factureMedRadio=0 OR mr.id_factureMedRadio IS NULL) ORDER BY mr.id_medradio');		
						...
						$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom mco, patients p WHERE p.numero=:num AND p.numero=mco.numero AND mco.numero=:num AND mco.id_consuConsom=:idconsu AND (mco.id_factureMedConsom=0 OR mco.id_factureMedConsom IS NULL) ORDER BY mco.id_medconsom');		
						...
						$resultMedMedoc=$connexion->prepare('SELECT *FROM med_medoc mdo, patients p WHERE p.numero=:num AND p.numero=mdo.numero AND mdo.numero=:num AND mdo.id_consuMedoc=:idconsu AND (mdo.id_factureMedMedoc=0 OR mdo.id_factureMedMedoc IS NULL) ORDER BY mdo.id_medmedoc');		
		...
		<table style="width:90%;margin-top:10px;" align="center">
			...
			<tr>
				<td style="text-align:left; margin: 10px auto auto; width:200px; background:#fff; padding-bottom:20px; border-bottom:1px solid #333;border-top:none;">
...
<?php
		...
		if(isset($_POST['prixtypeconsult']))
		{
			...
			for($i=0;$i<sizeof($addtc);$i++)
			{
				...
				if($comptPresta!=0)
				{
					if($ligne=$result->fetch())
					{
						$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixtc[$i].',p.prixprestaCCO='.$prixtcCCO[$i].' WHERE p.id_prestation='.$idprestatc[$i].'');
		...
		if(isset($_POST['idpresta']))
		{
			...
			for($i=0;$i<sizeof($add);$i++)
			{
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixmc[$i].',p.prixprestaCCO='.$prixmcCCO[$i].' WHERE p.id_prestation='.$idprestamc[$i].'');
		...
		if(isset($_POST['idprestaInf']))
		{
			...
			for($i=0;$i<sizeof($addInf);$i++)
			{
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixmi[$i].',p.prixprestaCCO='.$prixmiCCO[$i].' WHERE p.id_prestation='.$idprestami[$i].'');
		...
		if(isset($_POST['idprestaLab']))
		{
			...
			for($i=0;$i<sizeof($addLab);$i++)
			{
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixml[$i].',p.prixprestaCCO='.$prixmlCCO[$i].' WHERE p.id_prestation='.$idprestaml[$i].'');
		...
		if(isset($_POST['idprestaRad']))
		{
			...
			for($i=0;$i<sizeof($addRad);$i++)
			{
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixmr[$i].',p.prixprestaCCO='.$prixmrCCO[$i].' WHERE p.id_prestation='.$idprestamr[$i].'');
		...
		if(isset($_POST['idprestaConsom']))
		{
			...
			{			
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixmco[$i].',p.prixprestaCCO='.$prixmcoCCO[$i].' WHERE p.id_prestation='.$idprestaconsom[$i].'');
		...
		if(isset($_POST['idprestaMedoc']))
		{
			...
			{			
				...
				if($comptPresta!=0)
				{
					$updatePrixPresta=$connexion->query('UPDATE '.$presta_assuUpdate.' p SET p.prixpresta='.$prixmdo[$i].',p.prixprestaCCO='.$prixmdoCCO[$i].' WHERE p.id_prestation='.$idprestamedoc[$i].'');
	...
	<table style="width:100%; margin-bottom:-10px"> 
		<tr> 
			...
			<td style="text-align:right;width:33%;">
				...
				<form method="post" action="printBill.php?num=<?php echo $_GET['num'];?>&cashier=<?php echo $_SESSION['codeCash'];?>&dateconsu=<?php echo $dateconsu;?>&nomassurance=<?php echo $nomassurance;?>&idcardbill=<?php echo $idcardbill;?>&numpolicebill=<?php echo $numpolicebill;?>&adherentbill=<?php echo $adherentbill;?><?php if(isset($_GET['idconsu'])){ echo '&idconsu='.$_GET['idconsu'];}?><?php if(isset($_GET['idmed'])){ echo '&idmed='.$_GET['idmed'];}?><?php if(isset($_GET['idtypeconsu'])){ echo '&idtypeconsu='.$_GET['idtypeconsu'];}?><?php if(isset($vouchernum)){ echo '&vouchernum='.$vouchernum;}?><?php if(isset($_GET['idassu'])){ echo '&idassu='.$_GET['idassu'];}?><?php if(isset($idBilling)){ echo '&idbill='.$idBilling;}else{ echo '&idbill='.$_GET['idbill'];}?>&updatebill=ok&createBill=<?php echo $createBill;?>&finishbtn=ok" enctype="multipart/form-data" class="buttonBill">
