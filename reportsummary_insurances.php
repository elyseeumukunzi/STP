	...
	<table style="width:100%">
		...
		<tr>
			<td colspan=2 style="text-align:center;">
				<span style="text-align:center;background:#333;border-radius:40px;color:#eee;font-weight:400;padding:5px 50px">Powered by <img src="images/petilogo.png" style="width:5%;height:auto;"/> , a product of Ikorere Group Ltd. �2015-'.date('Y').', All Rights Reserved.</span>
			</td>
		</tr>
...
<?php
	...
	?>
		<table cellpadding=3 style="width:100%; margin:5px auto auto;">
			...
			<tr>
				...
				<td style="text-align:right; width:30%;">
					...
					<form method="post" action="reportsummary_insurances.php?dailydategnl=<?php echo $dailydategnl;?>&createReportPdf=ok" enctype="multipart/form-data" class="buttonBill">
		...
		if(isset($_GET['dailydategnl']))
		{
						//select all issurance available
			$SelectAssurance = $connexion->query('SELECT * FROM `assurances`');
			$SelectAssurance->setFetchMode(PDO::FETCH_OBJ);
		?>
		<div id="divGnlBillReport" style="font-weight:normal;">
			<table style="width:100%;" class="printPreview tablesorter" cellspacing="0"> 
				...
				<?php
					...
					$total = 0;
					$totalgnl = 0;
					while ($GetAllAssu = $SelectAssurance->fetch()) {
				?>
				<tbody>
					<tr style="padding-top:20px;">
					<td style="text-align:center;"><?php echo $compteur;?></td>
					<td style="text-align:center;"><?php echo $GetAllAssu->nomassurance;?></td>
					<td style="text-align:center;" >
						<?php 
							$resultGnlBillReport=$connexion->prepare('SELECT * FROM bills WHERE nomassurance=:nomassu AND '.$dailydategnl.'  ORDER BY datebill ASC');
							$resultGnlBillReport->execute(array(
							'nomassu'=> $GetAllAssu->nomassurance
							));
							...
							?>
									...
									if ($comptConsu!=0) {
										...
										while($ligneConsu=$resultConsu->fetch())
										{
											...
											$insuranceamount = $prixconsult.'<br>';
										    $totalConsult +=$insuranceamount; 
										}
										//echo $sum.'<br>';
									  $TotalGnConsulutation +=$totalConsult; 
									  }
									...
									if ($comptMedConsu !=0) {
										...
										while($ligneMedConsu=$resultMedConsu->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixconsu;
											$totalConsu += $insuranceamount; 
										}

										$TotalGnConsu +=$totalConsu;
									...
									if ($comptMedLabo !=0) {
										...
										while($ligneMedLabo=$resultMedLabo->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixlabo;
											$totalLabo += $insuranceamount; 
										}

										$TotalGnLabo+=$totalLabo ;
									...
									if ($comptMedRadio !=0) {
										...
										while($ligneMedRadio=$resultMedRadio->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixradio;
											$totalRadio += $insuranceamount; 
										}

										$TotalGnRadio+=$totalRadio;
									...
									if ($comptMedInf !=0) {
										...
										while($ligneMedInf=$resultMedInf->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixinf;
											$totalInf += $insuranceamount; 
										}
									...
									if ($comptMedConsom !=0) {
										...
										while($ligneMedConsom=$resultMedConsom->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixconsom;
											$totalConsom += $insuranceamount; 
										}

										$TotalGnConsom+=$totalConsom;
									...
									if ($comptMedMedoc !=0) {
										...
										while($ligneMedMedoc=$resultMedMedoc->fetch())//on recupere la liste des ??ents
										{
											...
											$insuranceamount = $prixmedoc;
											$totalMedDoc += $insuranceamount; 
										}
											$TotalGnMedDoc+=$totalMedDoc;
									}



									/*-------------------------------------------------------------------------For  Insurance Not Private------------------------------------------------------------------------------------------------------*/
											...
											if ($comptConsu!=0) {
												...
												while($ligneConsu=$resultConsu->fetch())
												{
													...
													$totalConsuAssu += $insuranceamount; 
												}
													$TotalGnConsulutation +=$totalConsuAssu; 
											}
									...
									if ($comptMedLabo !=0) {
										...
										while($ligneMedLabo=$resultMedLabo->fetch())//on recupere la liste des ??ents
										{
											...
											$totalLaboAssu += $insuranceamount; 
										}

										$TotalGnLabo+=$totalLaboAssu ;
									...
									if ($comptMedRadio !=0) {
										...
										while($ligneMedRadio=$resultMedRadio->fetch())//on recupere la liste des ??ents
										{
											...
											$totalRadioAssu += $insuranceamount; 
										}

										$TotalGnRadio+=$totalRadioAssu;
									...
									if ($comptMedInf !=0) {
										...
										while($ligneMedInf=$resultMedInf->fetch())//on recupere la liste des ??ents
										{
											...
											$totalInfAssu += $insuranceamount; 
										}

										$TotalGnInf+=$totalInfAssu;
									...
									if ($comptMedConsom !=0) {
										...
										while($ligneMedConsom=$resultMedConsom->fetch())//on recupere la liste des ??ents
										{
											...
											$totalConsomAssu += $insuranceamount; 
										}

										$TotalGnConsom+=$totalConsomAssu;
									...
									if ($comptMedMedoc !=0) {
										...
										while($ligneMedMedoc=$resultMedMedoc->fetch())//on recupere la liste des ??ents
										{
											...
											$totalMedDocAssu += $insuranceamount; 
										}
											$TotalGnMedDoc+=$totalMedDocAssu;
									}
						...
						<?php
							}//end of while on bills selection
							 // echo 'Consultation ='.$TotalGnConsulutation.'<br>';
							 // echo 'servi='.$TotalGnConsu.'<br>';
							 // echo 'labo='.$TotalGnLabo.'<br>';
							 // echo 'rado='.$TotalGnRadio.'<br>';
							 // echo 'inf='.$TotalGnInf.'<br>';
							 // echo 'consom='.$TotalGnConsom.'<br>';
							 // echo 'Meddoc='.$TotalGnMedDoc.'<br>';
							 $TotalAmount = $TotalGnConsulutation+$TotalGnConsu+$TotalGnLabo+$TotalGnRadio+$TotalGnInf+$TotalGnConsom+$TotalGnMedDoc;
							 echo "<b style='font-weight:bold;font-size:15px;'>".$TotalAmount."</b>";
						}//end if for bill selection
						?>
						</td>
					</tr>
				</tbody>
