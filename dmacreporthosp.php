		...
		<div id="divGnlBillReport" style="font-weight:normal;">
			...
			<table style="width:100%" class="printPreview tablesorter3" cellspacing="0"> 
				...
				<thead>
					<tr>
						...
						<th style="border-right: 1px solid #bbb;text-align:center;">Doctor name</th>
			...
			?>
				...
				<tr style="text-align:center;">
					...
					<td style="text-align: center;">
						<?php
							$id_consuHosp = $ligneGnlBillReport->id_consuHosp;

							if ($id_consuHosp != 0 ) {
							 	$seleconsuHosp = $connexion->prepare('SELECT * FROM consultations WHERE id_consu=:id_consu');
							 	$seleconsuHosp->execute(array(
							 		'id_consu'=>$id_consuHosp
							 	));
							 	$seleconsuHosp->setFetchMode(PDO::FETCH_OBJ);
							 	if ($ligneconsuHosp = $seleconsuHosp->fetch()) {
							 		$id_uM = $ligneconsuHosp->id_uM;

							 		$selenomdoc = $connexion->prepare('SELECT * FROM utilisateurs u, medecins m WHERE u.id_u=m.id_u AND u.id_u=:id_u ');
							 		$selenomdoc->execute(array(
							 			'id_u'=>$id_uM
							 		));
							 		$selenomdoc->setFetchMode(PDO::FETCH_OBJ);
							 		$lignenomDoc = $selenomdoc->fetch();

							 		echo $lignenomDoc->full_name;
							 	}
							} 
						?>						
					</td>
