			...
			?>
				...
				<thead>
					...
					<tr>
						...
						<th style="width:11%; border-right: 1px solid #bbb; text-align:center;"><?php echo 'Résultats';?></th>
				...
				$resultatsSouscategoPrestaLabo=$connexion->prepare('SELECT *FROM prestations_private p, med_labo ml WHERE ml.numero=:num AND p.id_prestation=ml.id_prestationExa '.$dailydateperso.' AND ml.examenfait=1 GROUP BY p.id_souscategopresta');		
				...
				while($ligneSouscategoPrestaLabo=$resultatsSouscategoPrestaLabo->fetch())
				{
					...
					{
						$nomSousCatego='Category: <span style="font-weight:bold">'.$ligneNomSouscatego->nomsouscatego.'</span>';
					}
				?>
