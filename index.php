...
try
{
 	...
 	if(isset($_POST['loginbtn']))
	{
		...
		if(!empty($s_number) AND !empty($password))
		{
			...
			$sqlO=$connexion->query("SELECT *FROM orthopedistes o, utilisateurs u WHERE u.password='$password' AND o.codeortho='$s_number' AND o.id_u=u.id_u");
			$sqlS=$connexion->query("SELECT *FROM stockkeeper s, utilisateurs u WHERE u.password='$password' AND s.codestock='$s_number' AND s.id_u=u.id_u");
			...
			if($comptidP!=0)
			{
				...
				if($ligne=$sqlP->fetch())
				{
					...
					$_SESSION['dataM']=$ligne->datamanager;
				...
				if($comptidD!=0)
				{
					...
					if($ligne=$sqlD->fetch())
					{
						...
						$getGrade=$connexion->prepare('SELECT *FROM categopresta_ins c, servicemed s WHERE c.id_categopresta=s.id_categopresta AND s.codemedecin=:codeM');
						$getGrade->execute(array(
						'codeM'=>$_SESSION['codeM']
						))or die( print_r($connexion->errorInfo()));
						
						$getGrade->setFetchMode(PDO::FETCH_OBJ);
						
						$comptGrade=$getGrade->rowCount();
						
						if($ligneGrade=$getGrade->fetch())
						{
							$_SESSION['idcatego']=$ligneGrade->id_categopresta;
							$_SESSION['nomcatego']=$ligneGrade->nomcategopresta;
						}	
							...
							if($comptidX!=0)
							{
								...
								if($comptidR!=0)
								{
									...
									if($comptidRecCash!=0)
									{
										...
										// if($annee >= '2021-04-25'){
										// 	echo '<script text="text/javascript">document.location.href="help.php?receptioniste=ok&francais=francais"</script>';
										// }else{
									...
									if($comptidC!=0)
									{
										...
										if($comptidA!=0)
										{
											...
											if($comptidM!=0)
											{
												...
												{
													if ($ligne->ceo==1) {
														$p='<b style="padding:5px 5px;border:1px solid #1bd037;border-radius:10px;">C.E.O</b>';
													}else{
														$p='<b style="padding:5px 5px;border:1px solid #1bd037;border-radius:10px;">Manager</b>';
													}
													$_SESSION['connect']=true;
													$_SESSION['id']=$ligne->id_u;
													$_SESSION['nom']=$p.' '.$ligne->nom_u;
													$_SESSION['prenom']=$ligne->prenom_u;
													$_SESSION['sexe']=$ligne->sexe;
													$_SESSION['province']=$ligne->province;
													$_SESSION['district']=$ligne->district;
												...
												if($comptidAcc!=0)
												{
													...
													if($comptidO!=0)
													{
														$sqlO->setFetchMode(PDO::FETCH_OBJ);
														...
														if($ligne=$sqlO->fetch())
														{
															...
															$_SESSION['codeO']=$ligne->codeortho;
															// echo $_SESSION['codeO'];
														...
														if($ligne=$sqlS->fetch())
														{
															...
															$_SESSION['codeS']=$ligne->codestock;
															// echo $_SESSION['codeO'];
															
															echo '<script text="text/javascript">document.location.href="stockrecording.php?francais=francais"</script>';
														}
