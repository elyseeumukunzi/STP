	...
	if(!isset($_GET['showresult']))
	{
		...
		$annee = date('Y').'-'.date('m').'-'.date('d');
			...
			*/
				...
				if($comptidR!=0)
				{
					$resultatsR=$connexion->query('SELECT *FROM utilisateurs u, patients p  consultations c WHERE c.dateconsu="'.$annee.'" AND u.id_u=p.id_u ORDER BY c.heureconsu  LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
				...
				if($comptidI!=0)
				{
					$resultatsR=$connexion->query('SELECT *FROM utilisateurs u, patients p, consultations c WHERE c.dateconsu="'.$annee.'" AND u.id_u=p.id_u AND p.numero=c.numero ORDER BY c.heureconsu DESC LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
